...
class gz::sim::systems::JointPositionControllerPrivate
{
  ...
  /// \brief Commanded joint position normalized [-1, 1]
  public: double jointPosCmdNormalized{0.0};

  /// \brief Max Scale (positive direction) For Joint Motion
  /// for revolute joint, this is in radian. for prismatic joint, this is in meters. 
  public: double maxScale{0.0};

  /// \brief Min Scale (positive direction) For Joint Motion
  /// for revolute joint, this is in radian. for prismatic joint, this is in meters. 
  ...
  /// \brief Print Debug to the Terminal Screen.
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  if (_sdf->HasElement("maxScale"))
  {
    this->dataPtr->maxScale = _sdf->Get<double>("maxScale");
  }
  else
  {
    gzerr << "Failed to get <maxScale>." << std::endl;
    return;
  }

  if (_sdf->HasElement("minScale"))
  {
    this->dataPtr->minScale = _sdf->Get<double>("minScale");
  }
  else
  {
    gzerr << "Failed to get <minScale>." << std::endl;
    return;
  }

  if (_sdf->HasElement("enableDebug"))
  {
    this->dataPtr->enableDebug = _sdf->Get<bool>("enableDebug");

    gzerr << "Debug: " << this->dataPtr->enableDebug << std::endl;
  }
  ...
  {
    this->dataPtr->jointPosCmdNormalized = _sdf->Get<double>("initial_position");
  }
  ...
  gzdbg << "initial_position: [" << this->dataPtr->jointPosCmdNormalized << "]"
         << std::endl;
}
    ...
    EntityComponentManager &_ecm)
{
  ...
  double error;
  {
    ...
    if(this->dataPtr->jointPosCmdNormalized > 0)
    {
      error = jointPosComp->Data().at(this->dataPtr->jointIndex) -
            this->dataPtr->maxScale * this->dataPtr->jointPosCmdNormalized;
    }
    else
    {
      error = jointPosComp->Data().at(this->dataPtr->jointIndex) -
            this->dataPtr->minScale * this->dataPtr->jointPosCmdNormalized;
    }
    ...
    /*    
      gzerr   << "JointName: "
              ...
              << " PosCommand: "
              << this->dataPtr->jointPosCmdNormalized << std::endl;
    */
...
void JointPositionControllerPrivate::OnCmdPos(const msgs::Double &_msg)
{
  ...
  this->jointPosCmdNormalized = _msg.data();
}
...
void JointPositionControllerPrivate::OnActuatorPos(const msgs::Actuators &_msg)
{
  ...
  this->jointPosCmdNormalized = static_cast<double>(_msg.position(this->actuatorNumber));
}
