...
#include "MySensor.hh"
#include "MySensorSystem.hh"

using namespace custom;

//////////////////////////////////////////////////
void MySensorSystem::PreUpdate(const gz::sim::UpdateInfo &,
  ...
  _ecm.EachNew<gz::sim::components::CustomSensor,
    ...
    [&](const gz::sim::Entity &_entity,
        ...
        const gz::sim::components::ParentEntity *_parent)->bool
      {
        ...
        if (data.Topic().empty())
        {
          std::string topic = scopedName(_entity, _ecm) + "/mysensor";
          data.SetTopic(topic);
        }

        gz::sensors::SensorFactory sensorFactory;
        auto sensor = sensorFactory.CreateSensor<custom::MySensor>(data);
        if (nullptr == sensor)
        {
          gzerr << "Failed to create mysensor [" << sensorScopedName << "]"
                 << std::endl;
          return false;
        }
...
//////////////////////////////////////////////////
void MySensorSystem::PostUpdate(const gz::sim::UpdateInfo &_info,
...
//////////////////////////////////////////////////
void MySensorSystem::RemoveSensorEntities(
    const gz::sim::EntityComponentManager &_ecm)
{
  _ecm.EachRemoved<gz::sim::components::CustomSensor>(
    [&](const gz::sim::Entity &_entity,
        const gz::sim::components::CustomSensor *)->bool
      {
        ...
        {
          gzerr << "Internal error, missing mysensor for entity ["
                         << _entity << "]" << std::endl;
        }
        return true;
      });
...
GZ_ADD_PLUGIN(MySensorSystem, gz::sim::System,
  MySensorSystem::ISystemPreUpdate,
  MySensorSystem::ISystemPostUpdate
)
