...
class gz::sim::systems::ModifiedMotorModelPrivate
{
  ...
  /// \brief Reference input to Actual Motor speed input multiplier
  /// using maxRotVelocity.
  public: double refMotorInputMultiplier = 1.0;
...
//////////////////////////////////////////////////
ModifiedMotorModel::ModifiedMotorModel()
  : dataPtr(std::make_unique<ModifiedMotorModelPrivate>())
{
}

//////////////////////////////////////////////////
void ModifiedMotorModel::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "ModifiedMotorModel plugin should be attached to a model "
  ...
  {
    gzerr << "ModifiedMotorModel found an empty jointName parameter. "
  ...
  {
    gzerr << "ModifiedMotorModel found an empty linkName parameter. "
           << "Failed to initialize.";
    return;
  }
  ...
  this->dataPtr->refMotorInputMultiplier = this->dataPtr->maxRotVelocity / 1000.0;
  ...
  this->dataPtr->node.Subscribe(topic,
      &ModifiedMotorModelPrivate::OnActuatorMsg, this->dataPtr.get());
}

//////////////////////////////////////////////////
void ModifiedMotorModel::PreUpdate(const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("ModifiedMotorModel::PreUpdate");
...
//////////////////////////////////////////////////
void ModifiedMotorModelPrivate::OnActuatorMsg(
...
//////////////////////////////////////////////////
void ModifiedMotorModelPrivate::UpdateForcesAndMoments(
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("ModifiedMotorModelPrivate::UpdateForcesAndMoments");
  ...
  switch (this->motorType)
  {
    ...
    default:  // MotorType::kVelocity
    {
      ...
      gzerr << "link: " << this->linkName << " refInput: " << this->refMotorInput << " MotVel: " << realMotorVelocity << " thrust: " << thrust << " maxSpeed: " << this->maxRotVelocity  << std::endl;
      ...
      refMotorRotVel = this->rotorVelocityFilter->UpdateFilter(
          this->refMotorInputMultiplier*this->refMotorInput, this->samplingTime);
...
GZ_ADD_PLUGIN(ModifiedMotorModel,
                    System,
                    ModifiedMotorModel::ISystemConfigure,
                    ModifiedMotorModel::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(ModifiedMotorModel,
                          "gz::sim::systems::ModifiedMotorModel")

// TODO(CH3): Deprecated, remove on version 8
GZ_ADD_PLUGIN_ALIAS(ModifiedMotorModel,
                          "ignition::gazebo::systems::ModifiedMotorModel")
