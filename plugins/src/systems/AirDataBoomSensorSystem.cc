...
#include "AirDataBoomSensorSystem.hh"

using namespace custom;
using namespace gz;
using namespace sim;

/// \brief Private AirDataBoomSensorSystem data class.
class custom::AirDataBoomSensorSystemPrivate
{

  /// \brief A map of model entities to their sensors
  public: std::unordered_map<gz::sim::Entity,
      std::unordered_map<gz::sim::Entity, std::unique_ptr<custom::AirDataBoomSensor>>> modelSensorMap;
  ...
  public: Entity aoaLinkEntity = kNullEntity;
  public: Entity aosLinkEntity = kNullEntity;
  ...
  public: void AddAirDataBoom(
    const gz::sim::EntityComponentManager &_ecm,
    const gz::sim::Entity _entity,
    const gz::sim::components::CustomSensor *_airDataBoom,
    const gz::sim::components::ParentEntity *_parent);

  /// \brief Create air data boom sensor
  /// \param[in] _ecm Immutable reference to ECM.
  ...
  /// \param[in] _ecm Immutable reference to ECM.
  public: void UpdateAirDataBoom(const gz::sim::EntityComponentManager &_ecm, gz::sim::Entity _entity);

  /// \brief Update vane link poses.
  /// \param[in] _info System update information.
  /// \param[in] _ecm The EntityComponentManager of the given simulation
  /// instance.
  public: void UpdateLinkPoses(const EntityComponentManager &_ecm, gz::sim::Entity _entity);

  /// \brief Remove air pressure sensors if their entities have been removed
  /// from simulation.
  /// \param[in] _ecm Immutable reference to ECM.
  public: void RemoveAirDataBoomEntities(const gz::sim::EntityComponentManager &_ecm, gz::sim::Entity _entity);
...
//////////////////////////////////////////////////
AirDataBoomSensorSystem::AirDataBoomSensorSystem() :
  System(), dataPtr(std::make_unique<AirDataBoomSensorSystemPrivate>())
{
}

//////////////////////////////////////////////////
AirDataBoomSensorSystem::~AirDataBoomSensorSystem() = default;

//////////////////////////////////////////////////
void AirDataBoomSensorSystem::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "AirDataBoomSensorSystem plugin should be attached to a model entity. "
           << "Failed to initialize." << std::endl;
    return;
  }

  //////////////////////////////////////////////////
  if (_sdf->HasElement("angle_of_attack_link_name"))
  {
    std::string aoa_link_name = _sdf->Get<std::string>("angle_of_attack_link_name");

    this->dataPtr->aoaLinkEntity = this->dataPtr->model.LinkByName(_ecm, aoa_link_name);

    gzerr << "aoaLinkEntity initialized to ["
           << this->dataPtr->aoaLinkEntity << "]" << std::endl;
  }
  else
  {
    gzerr << "aoaLinkEntity cannot be initialized with this name" << std::endl;
    return;
  }

  //////////////////////////////////////////////////
  if (_sdf->HasElement("angle_of_sideslip_link_name"))
  {
    std::string aos_link_name = _sdf->Get<std::string>("angle_of_sideslip_link_name");

    this->dataPtr->aosLinkEntity = this->dataPtr->model.LinkByName(_ecm, aos_link_name);

    gzerr << "aosLinkEntity initialized to ["
           << this->dataPtr->aosLinkEntity << "]" << std::endl;
  }
  else
  {
    gzerr << "aosLinkEntity cannot be initialized with this name" << std::endl;
    return;
  }
...
//////////////////////////////////////////////////
void AirDataBoomSensorSystem::PreUpdate(const gz::sim::UpdateInfo &,
...
//////////////////////////////////////////////////
void AirDataBoomSensorSystem::PostUpdate(const gz::sim::UpdateInfo &_info,
    const gz::sim::EntityComponentManager &_ecm)
{
  ...
  */
  ...
  if (!_info.paused)
  {
    ...
    if (it != this->dataPtr->modelSensorMap.end())
    {
      ...
      for (const auto &sensorIt : it->second)
      {
        ...
        this->dataPtr->UpdateAirDataBoom(_ecm, sensorIt.first);
        this->dataPtr->UpdateLinkPoses(_ecm, sensorIt.first);
  ...
  if (modelIt != this->dataPtr->modelSensorMap.end())
  {
    ...
    {
      this->dataPtr->RemoveAirDataBoomEntities(_ecm, entity);
    }
  }
}


gz::sim::Entity AirDataBoomSensorSystemPrivate::GetTopmostParentModel(const gz::sim::EntityComponentManager &_ecm, gz::sim::Entity _entity)
{
...
//////////////////////////////////////////////////
void AirDataBoomSensorSystemPrivate::AddAirDataBoom(
  const gz::sim::EntityComponentManager &_ecm,
  const gz::sim::Entity _entity,
  const gz::sim::components::CustomSensor *_airDataBoom,
  const gz::sim::components::ParentEntity *_parent)
{
  ...
  sdf::Sensor data = _airDataBoom->Data();
  data.SetName(sensorScopedName);

  // check topic
  if (data.Topic().empty())
  {
    std::string topic = scopedName(_entity, _ecm) + "/air_data_boom";
    data.SetTopic(topic);

    gzerr << "Topic set to " << topic << " for " << sensorScopedName << std::endl;
  }

  std::unique_ptr<custom::AirDataBoomSensor> sensor = 
      this->sensorFactory.CreateSensor<custom::AirDataBoomSensor>(data);
  if (nullptr == sensor)
  {
    gzerr << "Failed to create airdataboomsensor [" << sensorScopedName << "]"
            << std::endl;
  }
  ...
  sensor->SetModelName(parentModelName);
...
//////////////////////////////////////////////////
void AirDataBoomSensorSystemPrivate::CreateSensors(const gz::sim::EntityComponentManager &_ecm)
{
        ...
        const gz::sim::components::ParentEntity *_parent) -> bool
    {
     ...
     // gzerr << "Current model entity: " << this->modelEntity << std::endl;
      ...
      //  gzdbg << "Creating sensor for model entity: " << topmostModelEntity << " under current entity: " << this->modelEntity << std::endl;
        this->AddAirDataBoom(_ecm, _entity, _airDataBoom, _parent);
      }
      else
      {
...
bool AirDataBoomSensorSystemPrivate::IsDescendantOfModel(const gz::sim::EntityComponentManager &_ecm, gz::sim::Entity childEntity, gz::sim::Entity parentModelEntity)
{
...
//////////////////////////////////////////////////
void AirDataBoomSensorSystemPrivate::UpdateAirDataBoom(const gz::sim::EntityComponentManager &_ecm, gz::sim::Entity _entity)
{
...
//////////////////////////////////////////////////
void AirDataBoomSensorSystemPrivate::UpdateLinkPoses(const gz::sim::EntityComponentManager &_ecm, gz::sim::Entity _entity)
{

//  gzerr  << "Inside UpdateLinkPoses: " << std::endl;

  auto &sensors = this->modelSensorMap[this->modelEntity];
  auto it = sensors.find(_entity);
  if (it != sensors.end())
  {
    const math::Pose3d modelWorldPose = worldPose(this->modelEntity, _ecm);
    const math::Pose3d aoaLinkWorldPose = worldPose(this->aoaLinkEntity, _ecm);
    const math::Pose3d aosLinkWorldPose = worldPose(this->aosLinkEntity, _ecm);
    
    math::Quaternion aoaRotDiff = modelWorldPose.Rot().Inverse() * aoaLinkWorldPose.Rot();
    math::Quaternion aosRotDiff = modelWorldPose.Rot().Inverse() * aosLinkWorldPose.Rot();

    it->second->SetAngleOfAttack(aoaRotDiff.Pitch());
    it->second->SetAngleOfSideSlip(-aosRotDiff.Yaw()); // Corrected for NED Notation
  }
  else
  {
    gzerr << "Failed to update air speed: " << _entity << ". Entity not found." << std::endl;
...
//////////////////////////////////////////////////
void AirDataBoomSensorSystemPrivate::RemoveAirDataBoomEntities(
...
GZ_ADD_PLUGIN(AirDataBoomSensorSystem, 
  gz::sim::System,
  AirDataBoomSensorSystem::ISystemConfigure,
  AirDataBoomSensorSystem::ISystemPreUpdate,
  AirDataBoomSensorSystem::ISystemPostUpdate
)

GZ_ADD_PLUGIN_ALIAS(AirDataBoomSensorSystem, "custom::AirDataBoomSensorSystem")
